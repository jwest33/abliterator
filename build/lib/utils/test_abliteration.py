...
def eval_workspace_scores(
    model_path: str,
    jlens_vectors_path: str,
    prompts_path: str,
    limit: int = 50,
    threshold: float = 0.3,
    active_signal_threshold: float = 0.5,
    batch_size: int = 4,
    dtype: str = "float16",
    device: str = "cuda",
    output_dir: str = "./eval_results",
) -> dict:
    """Score prompts via the J-lens workspace projection (mid-computation refusal deliberation).

    Reports per-prompt workspace scores + threshold-based refusal predictions.
    Writes JSON to output_dir. Diagnostic use (does not generate responses).
    """
    import json
    import time
    from pathlib import Path
    from utils.workspace_refusal_detector import (
        WorkspaceRefusalDetector,
        WorkspaceRefusalDetectorConfig,
    )

    Path(output_dir).mkdir(parents=True, exist_ok=True)
    logger.info(f"Loading prompts from {prompts_path}")
    prompts = load_prompts(prompts_path)[:limit]
    ...
    model, tokenizer = load_model_and_tokenizer(
        model_path, device=device, dtype=dtype_map.get(dtype, torch.float16),
        trust_remote_code=True,
    )
    tokenizer.padding_side = "left"

    detector = WorkspaceRefusalDetector(
        model, tokenizer,
        WorkspaceRefusalDetectorConfig(
            jlens_vectors_path=jlens_vectors_path,
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        scored = detector.detect_refusal_with_scores(batch)
        for p, (is_ref, score) in zip(batch, scored):
            all_results.append(
                {"prompt": p, "workspace_refusal": bool(is_ref), "workspace_score": float(score)}
            )
    elapsed = time.perf_counter() - start

    num_refused = sum(1 for r in all_results if r["workspace_refusal"])
    rate = num_refused / len(all_results) if all_results else 0.0

    model_name = Path(model_path).name
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_path = Path(output_dir) / f"{model_name}-workspace-eval_{timestamp}.json"
    payload = {
        ...
        "threshold": threshold,
        "active_signal_threshold": active_signal_threshold,
        "workspace_refusal_rate": rate,
        "num_workspace_refusals": num_refused,
        "eval_time_seconds": elapsed,
        "active_layers": detector.active_layers,
        "results": all_results,
    }
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(payload, f, indent=2)
    logger.info(f"Workspace scores saved to {out_path}")
    logger.info(
        f"Workspace refusal rate: {num_refused}/{len(all_results)} = {rate:.3f} "
...
def eval_refusal_rates(
    ...
    seed: int = 42,
    output_dir: str = "./eval_results",
    detector_type: str = "loglik",
    jlens_vectors_path: Optional[str] = None,
    workspace_threshold: float = 0.3,
) -> dict:
    """
    ...
    # Create refusal detector (log-likelihood or workspace)
    if detector_type == "workspace":
        if not jlens_vectors_path:
            raise ValueError("detector_type='workspace' requires jlens_vectors_path")
        from utils.workspace_refusal_detector import (
            WorkspaceRefusalDetector,
            WorkspaceRefusalDetectorConfig,
        )
        logger.info(f"Creating workspace refusal detector from {jlens_vectors_path}")
        refusal_detector = WorkspaceRefusalDetector(
            model,
            tokenizer,
            WorkspaceRefusalDetectorConfig(
                jlens_vectors_path=jlens_vectors_path,
                projection_threshold=workspace_threshold,
            ),
        )
    else:
...
Examples:
  ...
  # Quality eval (perplexity + KL divergence)
  python test_abliteration.py quality --model_path /path/to/abliterated --original_path /path/to/original
        """
    )
    ...
    eval_parser.add_argument(
        "--detector",
        type=str,
        choices=["loglik", "workspace"],
        default="loglik",
        help="Refusal detector: 'loglik' (default, log-likelihood of refusal anchors) or 'workspace' (J-lens workspace projection, requires --jlens-vectors)",
    )
    eval_parser.add_argument(
        "--jlens-vectors",
        type=str,
        default=None,
        help="Path to jlens_vectors.pt (required for --detector workspace)",
    )
    eval_parser.add_argument(
        "--workspace-threshold",
        type=float,
        default=0.3,
        help="Refusal threshold for workspace detector (cosine similarity, default: 0.3)",
    )

    # Workspace subcommand: raw workspace scores for diagnostic use
    workspace_parser = subparsers.add_parser(
        "workspace",
        help="Score prompts via J-lens workspace projection (diagnostic; reports scores, no generation)",
    )
    workspace_parser.add_argument("--model_path", "-m", type=str, required=True)
    workspace_parser.add_argument("--jlens-vectors", type=str, required=True, help="Path to jlens_vectors.pt")
    workspace_parser.add_argument("--prompts", type=str, required=True, help="Path to prompts file (one per line)")
    workspace_parser.add_argument("--limit", type=int, default=50, help="Max prompts to score")
    workspace_parser.add_argument("--threshold", type=float, default=0.3)
    workspace_parser.add_argument("--active-signal-threshold", type=float, default=0.5)
    workspace_parser.add_argument("--batch_size", "-b", type=int, default=4)
    workspace_parser.add_argument(
        "--dtype", type=str, choices=["float16", "bfloat16", "float32"], default="float16"
    )
    workspace_parser.add_argument(
    ...
    # Quality subcommand - perplexity and KL divergence comparison
    quality_parser = subparsers.add_parser(
        "quality",
        help="Compare perplexity and KL divergence between original and abliterated models",
    )
    ...
    quality_parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to reference prompts file (default: harmless.txt)",
    )
    quality_parser.add_argument(
        "--limit", "-l",
        type=int,
    ...
    quality_parser.add_argument(
        ...
        type=int,
        default=200,
        help="Top-k tokens for KL approximation (default: 200)",
    )
    quality_parser.add_argument(
        "--output_dir", "-o",
        type=str,
    ...
    if args.command == "eval":
        eval_refusal_rates(
            ...
            jlens_vectors_path=args.jlens_vectors,
            workspace_threshold=args.workspace_threshold,
        )

    elif args.command == "workspace":
        eval_workspace_scores(
            model_path=args.model_path,
            jlens_vectors_path=args.jlens_vectors,
            prompts_path=args.prompts,
            limit=args.limit,
            threshold=args.threshold,
            active_signal_threshold=args.active_signal_threshold,
            batch_size=args.batch_size,
            dtype=args.dtype,
            device=args.device,
            output_dir=args.output_dir,
        ...
        prompts = load_prompts_for_quality_eval(
            path=args.prompts,
            num_prompts=args.limit,
        )

        logger.info(f"Running quality eval with {len(prompts)} prompts")
        result = compare_quality(
            base_model_path=args.original_path,
            abliterated_model_path=args.model_path,
            prompts=prompts,
            batch_size=args.batch_size,
            top_k=args.top_k,
            device=args.device,
        ...
        logger.info("=" * 60)
        logger.info(f"Base model:         {args.original_path}")
        logger.info(f"Abliterated model:  {args.model_path}")
        logger.info(f"Prompts evaluated:  {result.num_prompts}")
        logger.info(f"Base perplexity:    {result.base_perplexity:.2f}")
        logger.info(f"Abl. perplexity:    {result.abliterated_perplexity:.2f}")
        logger.info(f"Perplexity delta:   {result.perplexity_delta:+.2f}")
        logger.info(f"Perplexity ratio:   {result.perplexity_ratio:.4f}x")
        logger.info(f"Mean KL divergence: {result.mean_kl_divergence:.4f} nats")
