...
class AbliterationConfig:
    ...
    # J-lens (Jacobian Lens / J-space) direction restriction
    # Projects refusal direction onto the per-layer J-space basis before normalization,
    # keeping only the causally-active component (paper: ~6-10% of variance carries flexible behavior).
    use_jlens_restriction: bool = False
    jlens_vectors_path: Optional[str] = None       # If set, load precomputed vectors; else compute inline
    jlens_basis_rank: int = 16                     # Max rank of QR-orthonormalized concept basis
    jlens_min_projection_ratio: float = 0.1        # Fallback: if restriction retains < ratio, use unrestricted
    jlens_num_prompts: int = 32                    # For inline computation
    ...
    # Gabliteration: multi-directional SVD-based abliteration (arxiv 2512.18901)
    use_gabliteration: bool = False
    gab_num_directions: int = 2              # k: number of SVD directions (1-10, paper recommends 1-3)
    gab_ridge_lambda: float = 0.1            # Ridge regularization for projection matrix
    gab_layer_scaling_beta: float = 0.5      # Adaptive layer scaling curvature (0=uniform)
    gab_skip_first_layers: int = 2           # Skip first N layers
    gab_skip_last_layers: int = 2            # Skip last N layers
...
class RefusalDirections:
    ...
    # Gabliteration: multi-directional SVD support
    multi_directions: Optional[dict[int, torch.Tensor]] = None  # layer_idx -> R matrix [hidden_dim, k]
    singular_values: Optional[dict[int, torch.Tensor]] = None  # layer_idx -> top-k singular values [k]
    mean_multi_direction: Optional[torch.Tensor] = None  # Mean R matrix [hidden_dim, k]
    ...
    def save(self, path: str):
        ...
        save_dict = {
            ...
            # Gabliteration fields
            "multi_directions": {k: v.cpu() for k, v in self.multi_directions.items()} if self.multi_directions else None,
            "singular_values": {k: v.cpu() for k, v in self.singular_values.items()} if self.singular_values else None,
            "mean_multi_direction": self.mean_multi_direction.cpu() if self.mean_multi_direction is not None else None,
        }
    ...
    def load(cls, path: str) -> "RefusalDirections":
        ...
        return cls(
            ...
            # Gabliteration fields (with backwards compatibility)
            multi_directions=data.get("multi_directions"),
            singular_values=data.get("singular_values"),
            mean_multi_direction=data.get("mean_multi_direction"),
        )
...
def get_transformer_layers(model):
    """Get the transformer layers from a HF model, across supported architectures.

    Returns the ModuleList of decoder layers. Raises ValueError with a structure
    dump for unsupported architectures. Shared between ActivationExtractor and
    J-lens extraction so both stay in sync.
    """
    # Vision-Language and standard text (model.model.*)
    ...
    # VL variants exposing language_model directly
    if hasattr(model, "language_model"):
        ...
        if hasattr(model.language_model, "transformer") and hasattr(model.language_model.transformer, "layers"):
            return model.language_model.transformer.layers
    ...
    # Structure dump for unsupported architectures
    lines = []

    def _explore(obj, prefix="", depth=0, max_depth=3):
        if depth >= max_depth:
            return
        for name in dir(obj):
            ...
            try:
                ...
                if isinstance(attr, torch.nn.ModuleList):
                    ...
                    _explore(attr, prefix + "  ", depth + 1, max_depth)
            except Exception:
                pass

    _explore(model)
    raise ValueError(
        f"Could not find layers in model architecture: {type(model)}\n"
        f"Model structure:\n{chr(10).join(lines[:50])}\n"
        f"Please add support for this architecture in get_transformer_layers()"
    )
...
def compute_refusal_directions(
    ...
    # J-lens (J-space) restriction: prepare per-layer basis if enabled.
    # Loaded here (not per-layer) so we can log up-front and fail fast if the file is missing.
    jlens_basis_by_layer: Optional[dict] = None
    jlens_restriction_ratios: dict[int, float] = {}
    if config.use_jlens_restriction:
        try:
            from src.jlens import (
                ...
                build_jlens_basis,
                restrict_direction_to_jlens_subspace,  # noqa: F401 (used below)
            )
            if config.jlens_vectors_path:
                ...
                jl_cfg = JLensConfig(
                    ...
                    gradient_checkpointing=config.jlens_grad_checkpoint,
                    device=config.device,
                    dtype=config.dtype,
            ...
            # Rebuild basis at the configured rank in case the cached file used a different one
            jlens_basis_by_layer = build_jlens_basis(
                jlens_obj.concept_vectors, rank=config.jlens_basis_rank
            )
            covered = sorted(jlens_basis_by_layer.keys())
            logger.info(f"J-lens restriction enabled: basis available for {len(covered)} layers")
        except Exception as e:
            logger.warning(
                f"J-lens restriction requested but setup failed ({e}); continuing without restriction"
            )
            jlens_basis_by_layer = None

    # Gabliteration: compute multi-directional SVD-based directions
    multi_directions = None
    gab_singular_values = None
    mean_multi_direction = None

    if config.use_gabliteration:
        logger.info(f"Computing Gabliteration multi-directional SVD (k={config.gab_num_directions})...")
        multi_directions, gab_singular_values, mean_multi_direction = compute_gabliteration_directions(
            harmful_activations, harmless_activations, config, extraction_layers,
        )

    # Compute refusal directions as mean difference
    # (Always computed: used as backward-compat fallback, for biprojection quality scoring, etc.)
    directions = {}
    for layer_idx in extraction_layers:
        ...
        if has_harmful and has_harmless:
            # For Gabliteration: use 1st singular vector as the primary direction for backward compat
            if config.use_gabliteration and multi_directions and layer_idx in multi_directions:
                direction = multi_directions[layer_idx][:, 0]  # First column = 1st singular vector
                if config.normalize_directions:
                    direction = F.normalize(direction, dim=0)
                directions[layer_idx] = direction.to(config.dtype)
                logger.debug(f"Layer {layer_idx}: using 1st SVD direction (norm = {direction.norm().item():.4f})")
                continue
            ...
            # J-lens restriction: project onto the J-space subspace to isolate
            # the causally-active refusal component (workspace paper). Falls back
            # to the unrestricted direction if too little norm survives.
            if jlens_basis_by_layer is not None and layer_idx in jlens_basis_by_layer:
                from src.jlens import restrict_direction_to_jlens_subspace
                direction, ratio = restrict_direction_to_jlens_subspace(
                    direction,
                    jlens_basis_by_layer[layer_idx],
                    min_projection_ratio=config.jlens_min_projection_ratio,
                )
                jlens_restriction_ratios[layer_idx] = ratio
    ...
    metadata = {
        ...
        # Gabliteration metadata
        "use_gabliteration": config.use_gabliteration,
        "gab_num_directions": config.gab_num_directions if config.use_gabliteration else None,
        "gab_ridge_lambda": config.gab_ridge_lambda if config.use_gabliteration else None,
        # J-lens restriction metadata
        "use_jlens_restriction": config.use_jlens_restriction and jlens_basis_by_layer is not None,
        "jlens_basis_rank": config.jlens_basis_rank if config.use_jlens_restriction else None,
        "jlens_min_projection_ratio": config.jlens_min_projection_ratio if config.use_jlens_restriction else None,
        "jlens_restriction_ratios": (
            {int(k): float(v) for k, v in jlens_restriction_ratios.items()}
            if jlens_restriction_ratios else None
        ),
    }
    ...
    return RefusalDirections(
        ...
        multi_directions=multi_directions,
        singular_values=gab_singular_values,
        mean_multi_direction=mean_multi_direction,
    )
...
def compute_gabliteration_directions(
    harmful_activations: dict[int, torch.Tensor],
    harmless_activations: dict[int, torch.Tensor],
    config: AbliterationConfig,
    ...
    """
    Compute multi-directional refusal directions via SVD of paired activation differences.

    Instead of a single mean-difference vector, this forms a difference matrix D where
    D[i,:] = harmful[i,:] - harmless[i,:] for paired samples, then extracts the top-k
    right singular vectors as orthogonal refusal directions.

    Args:
        harmful_activations: layer_idx -> [n_harmful, hidden_dim] activation tensors
        harmless_activations: layer_idx -> [n_harmless, hidden_dim] activation tensors
        config: AbliterationConfig with gabliteration parameters
        extraction_layers: List of layer indices to process

    Returns:
        Tuple of (multi_directions, singular_values, mean_multi_direction) where:
        - multi_directions: layer_idx -> R matrix [hidden_dim, k]
        - singular_values: layer_idx -> top-k singular values [k]
        - mean_multi_direction: Mean R matrix [hidden_dim, k] averaged across layers
    """
    k = config.gab_num_directions
    use_float64 = config.use_float64_subtraction

    multi_directions = {}
    singular_values_dict = {}

    for layer_idx in extraction_layers:
        if layer_idx not in harmful_activations or layer_idx not in harmless_activations:
            continue

        harmful_acts = harmful_activations[layer_idx]
        harmless_acts = harmless_activations[layer_idx]

        # Truncate to paired samples
        n = min(harmful_acts.shape[0], harmless_acts.shape[0])
        if n < k:
            logger.warning(f"Layer {layer_idx}: only {n} paired samples, need at least {k} for SVD, skipping")
            continue

        harmful_paired = harmful_acts[:n]
        harmless_paired = harmless_acts[:n]

        # Form difference matrix D[i,:] = harmful[i,:] - harmless[i,:]
        if use_float64:
            D = harmful_paired.double() - harmless_paired.double()
        else:
            D = harmful_paired.float() - harmless_paired.float()

        # Optional: orthogonalize each row of D against harmless mean
        # This removes the "general helpfulness magnitude" confound from each paired difference
        if config.use_projected_refusal:
            harmless_mean = harmless_paired.mean(dim=0)
            if use_float64:
                harmless_mean = harmless_mean.double()
            else:
                harmless_mean = harmless_mean.float()
            harmless_norm = F.normalize(harmless_mean, dim=0)
            # D_projected[i,:] = D[i,:] - (D[i,:] . harmless_norm) * harmless_norm
            proj_coeffs = D @ harmless_norm  # [n]
            D = D - proj_coeffs.unsqueeze(1) * harmless_norm.unsqueeze(0)

        # SVD of difference matrix: D = U @ S @ Vt
        # We want the top-k right singular vectors (columns of V, rows of Vt)
        try:
            U, S, Vt = torch.linalg.svd(D, full_matrices=False)
        except Exception as e:
            logger.warning(f"Layer {layer_idx}: SVD failed ({e}), skipping")
            continue

        # Extract top-k directions: R = Vt[:k, :].T -> [hidden_dim, k]
        actual_k = min(k, Vt.shape[0])
        R = Vt[:actual_k, :].T.float()  # [hidden_dim, actual_k]

        # Normalize columns of R (should already be unit vectors from SVD, but ensure)
        R = F.normalize(R, dim=0)

        multi_directions[layer_idx] = R.to(config.dtype)
        singular_values_dict[layer_idx] = S[:actual_k].float().to(config.dtype)

        # Log explained variance
        total_variance = (S ** 2).sum().item()
        explained_variance = (S[:actual_k] ** 2).sum().item()
        explained_pct = (explained_variance / total_variance * 100) if total_variance > 0 else 0
        logger.debug(
            f"Layer {layer_idx}: top-{actual_k} SVD directions explain "
            f"{explained_pct:.1f}% of variance (S={S[:actual_k].tolist()})"
        )

    if not multi_directions:
        logger.warning("No Gabliteration directions computed for any layer")
        return {}, {}, None

    # Compute mean_multi_direction by averaging R matrices across layers, then re-orthogonalize
    R_stack = torch.stack(list(multi_directions.values()))  # [num_layers, hidden_dim, k]
    R_mean = R_stack.float().mean(dim=0)  # [hidden_dim, k]

    # Re-orthogonalize via SVD to ensure columns are orthonormal
    try:
        U_mean, S_mean, Vt_mean = torch.linalg.svd(R_mean, full_matrices=False)
        mean_multi_direction = U_mean[:, :k].to(config.dtype)  # [hidden_dim, k]
    except Exception:
        # Fallback: just normalize columns
        mean_multi_direction = F.normalize(R_mean, dim=0).to(config.dtype)

    logger.info(
        f"Computed Gabliteration directions for {len(multi_directions)} layers "
        f"(k={k}, ridge_lambda={config.gab_ridge_lambda})"
    )

    return multi_directions, singular_values_dict, mean_multi_direction


def compute_gabliteration_layer_scaling(
    num_layers: int,
    alpha_base: float,
    beta: float = 0.5,
    skip_first: int = 2,
    ...
    """
    Compute per-layer scaling for Gabliteration.

    Uses the paper formula: alpha_l = alpha_base * (1 + beta * (1 - |xi_l|))
    where xi_l normalizes layer position to [-1, 1] within the effective range.
    Maximum at the center of the effective range, tapered at boundaries.

    Args:
        num_layers: Total number of layers in the model
        alpha_base: Base ablation strength (direction_multiplier)
        beta: Curvature parameter (0=uniform, higher=more peaked)
        skip_first: Number of first layers to skip (alpha=0)
        skip_last: Number of last layers to skip (alpha=0)

    Returns:
        Dict mapping layer_idx -> effective alpha for that layer
    """
    layer_scaling = {}

    effective_start = skip_first
    effective_end = num_layers - skip_last

    if effective_start >= effective_end:
        logger.warning(
            f"Gabliteration skip_first={skip_first} + skip_last={skip_last} >= num_layers={num_layers}, "
            "no layers in effective range"
        )
        return layer_scaling

    effective_count = effective_end - effective_start

    for layer_idx in range(num_layers):
        if layer_idx < effective_start or layer_idx >= effective_end:
            layer_scaling[layer_idx] = 0.0
            continue

        # Normalize position to [-1, 1] within effective range
        # 0 = start of effective range, 1 = end of effective range
        t = (layer_idx - effective_start) / max(effective_count - 1, 1)
        xi = 2.0 * t - 1.0  # Map [0,1] -> [-1,1]

        # Paper formula: maximum at center (xi=0), tapered at edges (|xi|=1)
        alpha_l = alpha_base * (1.0 + beta * (1.0 - abs(xi)))
        layer_scaling[layer_idx] = alpha_l

    return layer_scaling


def apply_gabliteration_projection(
    weight: torch.Tensor,
    R: torch.Tensor,
    alpha: float = 1.0,
    ...
    """
    Apply multi-directional ridge-regularized projection to a weight matrix.

    Implements: W_new = W - alpha * (W @ P) where
    P = R @ (R^T R + lambda I_k)^{-1} @ R^T

    Efficient computation without materializing d x d matrix:
        WR = W @ R             [out, k]
        G_inv = inv(R^T R + lambda I_k)  [k, k]
        adjustment = (WR @ G_inv) @ R^T  [out, in]
        W_new = W - alpha * adjustment

    Args:
        weight: Weight matrix [out_features, in_features]
        R: Matrix of refusal directions [hidden_dim, k] (columns are directions)
        alpha: Ablation strength (effective per-layer alpha)
    ...
    R_float = R.to(device=weight.device, dtype=torch.float32)
    k = R_float.shape[1]

    if preserve_norm:
        original_norm = weight_float.norm()

    # Determine alignment: R's first dim should match one of weight's dims
    if R_float.shape[0] == weight_float.shape[1]:
        # R aligns with input dimension (typical case)
        # WR = W @ R  [out_features, k]
        WR = weight_float @ R_float

        # G = R^T @ R + lambda * I_k  [k, k]
        G = R_float.T @ R_float + ridge_lambda * torch.eye(k, device=weight.device, dtype=torch.float32)

        # G_inv = inv(G)  [k, k]
        G_inv = torch.linalg.inv(G)

        # adjustment = (WR @ G_inv) @ R^T  [out_features, in_features]
        adjustment = (WR @ G_inv) @ R_float.T

        weight_new = weight_float - alpha * adjustment

    elif R_float.shape[0] == weight_float.shape[0]:
        # R aligns with output dimension
        # R^T @ W  [k, in_features]
        RtW = R_float.T @ weight_float

        # G = R^T @ R + lambda * I_k  [k, k]
        G = R_float.T @ R_float + ridge_lambda * torch.eye(k, device=weight.device, dtype=torch.float32)
        G_inv = torch.linalg.inv(G)

        # adjustment = R @ (G_inv @ R^T @ W)  [out_features, in_features]
        adjustment = R_float @ (G_inv @ RtW)

        weight_new = weight_float - alpha * adjustment

    else:
        logger.warning(
            f"R shape {R_float.shape} doesn't match weight shape {weight_float.shape}, skipping"
        )
        return weight
...
def abliterate_model(
    ...
    mode_parts = []
    if config.use_gabliteration:
        mode_parts.append(f"Gabliteration (k={config.gab_num_directions})")
    ...
    # Gabliteration incompatibility warnings
    if config.use_gabliteration:
        if config.use_per_neuron_norm:
            logger.warning("Gabliteration is incompatible with per-neuron norm preservation; "
                           "per-neuron norm will be ignored for layers with multi-directional R matrices")
        if null_space_projector is not None:
            logger.warning("Gabliteration does not yet compose with null-space constraints; "
                           "null-space will be ignored for layers with multi-directional R matrices")
        if config.use_adaptive_weighting:
            logger.warning("Gabliteration has its own layer scaling; --adaptive-weighting is superseded")
    ...
    # Gabliteration layer scaling (supersedes adaptive weighting when enabled)
    gab_layer_scaling = None
    if config.use_gabliteration:
        layer_indices = [get_layer_index_from_name(n) for n in linear_names]
        max_layer = max((i for i in layer_indices if i is not None), default=0)
        gab_num_layers = max_layer + 1

        gab_layer_scaling = compute_gabliteration_layer_scaling(
            gab_num_layers,
            alpha_base=config.direction_multiplier,
            beta=config.gab_layer_scaling_beta,
            skip_first=config.gab_skip_first_layers,
            skip_last=config.gab_skip_last_layers,
        )
        logger.info(
            f"Gabliteration layer scaling: beta={config.gab_layer_scaling_beta}, "
            f"skip_first={config.gab_skip_first_layers}, skip_last={config.gab_skip_last_layers}"
        )
    ...
    for name in tqdm(linear_names, desc="Abliterating layers"):
        ...
        # Gabliteration layer scaling supersedes other weighting when enabled
        if config.use_gabliteration and gab_layer_scaling is not None and layer_idx is not None:
            gab_alpha = gab_layer_scaling.get(layer_idx, 0.0)
            if gab_alpha == 0.0:
                skipped_count += 1
                continue
            effective_multiplier = gab_alpha
        elif layer_weights is not None and layer_idx is not None:
            if use_target_map:
        ...
        # Hybrid architecture: apply differentiated layer weighting (composes multiplicatively)
        ...
        try:
            # Gabliteration: multi-directional ridge-regularized projection
            R_matrix = None
            if config.use_gabliteration and directions.multi_directions:
                R_matrix = directions.multi_directions.get(layer_idx)
                if R_matrix is None and directions.mean_multi_direction is not None:
                    R_matrix = directions.mean_multi_direction

            if config.use_gabliteration and R_matrix is not None:
                new_weight = apply_gabliteration_projection(
                    weight,
                    R_matrix.to(config.device),
                    alpha=effective_multiplier,
                    ridge_lambda=config.gab_ridge_lambda,
...
def run_abliteration(config: AbliterationConfig):
    ...
    if config.use_gabliteration:
        features.append(f"Gabliteration (k={config.gab_num_directions})")
        if config.use_adaptive_weighting:
            logger.warning("Gabliteration supersedes --adaptive-weighting (Gabliteration has its own layer scaling)")
    ...
    config_save = {
        ...
        "save_directions": config.save_directions,
        "load_directions_path": config.load_directions_path,
        # Prompt info
        ...
        "hybrid_full_attn_weight": config.hybrid_full_attn_weight if config.hybrid_strategy != "uniform" else None,
        "hybrid_linear_attn_weight": config.hybrid_linear_attn_weight if config.hybrid_strategy != "uniform" else None,
        "hybrid_skip_recurrent_proj": config.hybrid_skip_recurrent_proj if config.hybrid_strategy != "uniform" else None,
        "hybrid_skip_state_proj": config.hybrid_skip_state_proj if config.hybrid_strategy != "uniform" else None,
        # Gabliteration (multi-directional SVD)
        "use_gabliteration": config.use_gabliteration,
        "gab_num_directions": config.gab_num_directions if config.use_gabliteration else None,
        "gab_ridge_lambda": config.gab_ridge_lambda if config.use_gabliteration else None,
        "gab_layer_scaling_beta": config.gab_layer_scaling_beta if config.use_gabliteration else None,
        "gab_skip_first_layers": config.gab_skip_first_layers if config.use_gabliteration else None,
        "gab_skip_last_layers": config.gab_skip_last_layers if config.use_gabliteration else None,
        # KL monitoring
        "use_kl_monitoring": config.use_kl_monitoring,
        "use_kl_auto_tune": config.use_kl_auto_tune,
        "kl_reference_prompts_path": config.kl_reference_prompts_path if (config.use_kl_monitoring or config.use_kl_auto_tune) else None,
        ...
        "kl_batch_size": config.kl_batch_size if (config.use_kl_monitoring or config.use_kl_auto_tune) else None,
        "kl_threshold": config.kl_threshold if config.use_kl_auto_tune else None,
        "kl_search_min": config.kl_search_min if config.use_kl_auto_tune else None,
        "kl_search_max": config.kl_search_max if config.use_kl_auto_tune else None,
        "kl_search_tolerance": config.kl_search_tolerance if config.use_kl_auto_tune else None,
        "kl_max_search_iterations": config.kl_max_search_iterations if config.use_kl_auto_tune else None,
    }
